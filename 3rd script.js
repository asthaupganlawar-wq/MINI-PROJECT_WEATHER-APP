async function fetchWeather() {
  ...
  saveWeather(record);
  displayWeather(); // now appends instead of replacing
...
// ✅ Updated to APPEND cards instead of replacing
function displayWeather() {
  const container = document.getElementById("weatherCards");
  const data = JSON.parse(localStorage.getItem('weatherData')) || [];

  // Remove this line: container.innerHTML = "";
  // Instead, we only clear duplicates
  container.innerHTML = ""; // keeps it clean from previous session only once
  data.forEach((rec, i) => {
